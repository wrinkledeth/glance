...
def fetch_tiktok(url: str, progress: Progress | None = None) -> str:
    """Fetch a TikTok video and return structured text for summarization."""
    _emit(progress, "fetching tiktok metadata/comments with yt-dlp")
    info = _dump_info(url)

    parts: list[str] = ["Source: TikTok"]
...
def _metadata_lines(info: dict[str, Any]) -> list[str]:
    ...
    author = _first_text(info, "channel", "uploader", "uploader_id")
    if author:
